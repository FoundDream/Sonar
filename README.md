...
Sonar 不是一次性吐出结果，而是由 LLM 驱动的 Coordinator Agent 自主协调多个专家 Agent，按 `Analyze → Research → Review → Synthesize` 流程推进。
